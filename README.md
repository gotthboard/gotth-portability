...
metadata, limit, and offset preflights happen before record output and leave it
reusable. `Next` nil context/sink and entry-cancellation preflights likewise
consume no frame and leave a live importer reusable. After preflight passes and
record/frame I/O begins, a failure poisons the object even if cancellation
prevents the first callback; resume from its prior checkpoint instead of
retrying it.
